...
trait SbtPomKeys {
  ...
  lazy val settingsLocation = SettingKey[File](
    "mvn-settings-location",
    "The location of the user settings file. Defaults to `~/.m2/settings.xml`"
  )
  lazy val profiles = SettingKey[Seq[String]]("mvn-profiles", "List of maven profiles to be applied.")
  lazy val mvnLocalRepository = SettingKey[File](
    "mvn-local-repository",
    "The location of the maven local repository we can use to cache artifacts."
  )
